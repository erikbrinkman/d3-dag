...
tape("simpleLeft() works for square", (test) => {
  const layout = d3_dag.arquint().columnAssignment(d3_dag.columnSimpleLeft());
...
tape("simpleLeft() works for dag", (test) => {
  const layout = d3_dag.arquint().columnAssignment(d3_dag.columnSimpleLeft());
  ...
  test.equals(five.columnIndex, 1 - three.columnIndex);
  test.end();
});
