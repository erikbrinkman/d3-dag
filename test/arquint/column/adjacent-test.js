...
tape("left adjacent() works for square", (test) => {
  const layout = d3_dag
    .arquint()
    .columnAssignment(d3_dag.columnAdjacent().center(false));
...
tape("left adjacent() works for dag", (test) => {
  const layout = d3_dag
    .arquint()
    .columnAssignment(d3_dag.columnAdjacent().center(false));
...
tape("center adjacent() works for square", (test) => {
  const layout = d3_dag
    .arquint()
    .columnAssignment(d3_dag.columnAdjacent().center(true));
  const dag = layout(load("square"));
  const [zero, one, two, three] = dag.descendants().sort((a, b) => a.id - b.id);
...
tape("center adjacent() works for dag", (test) => {
  const layout = d3_dag
    .arquint()
    .columnAssignment(d3_dag.columnAdjacent().center(true));
  const dag = layout(load("dag"));
