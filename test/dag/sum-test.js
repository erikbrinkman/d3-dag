...
tape("sum() is correct for square", test => {
  ...
  test.deepEquals(dag.nodes().map(n => n.value), [6, 4, 5, 3]);
  test.end()
});

tape("sum() is correct for N", test => {
  const dag = en.sum(d => parseInt(d.id))
  test.deepEquals(dag.nodes().map(n => n.value), [2, 6, 2, 3]);
  test.end()
});

tape("sum() is correct for X", test => {
  const dag = ex.sum(d => parseInt(d.id))
  test.deepEquals(dag.nodes().map(n => n.value), [19, 19, 20, 18, 4, 11, 6]);
  test.end()
});
