...
tape("dagHierarchy() parses the stratify square", test => {
  const root = d3_dag.dagHierarchy()
...
tape("dagHierarchy() fails with invalid root", test => {
  ...
  one.children = [two];
  two.children = [one];
  test.throws(() => d3_dag.dagHierarchy()(one), /dag contained a cycle: 1 -> 2 -> 1/);
  test.end();
});
...
tape("dagHierarchy() fails with null id", test => {
  test.throws(() => d3_dag.dagHierarchy()({id: "\0"}), /id contained null character/);
  test.end();
});

tape("dagHierarchy() fails with null data", test => {
  test.throws(() => d3_dag.dagHierarchy().id(() => "0").children(() => [])(null), /falsy data/);
  test.end();
});

tape("dagHierarchy() fails with false data", test => {
  test.throws(() => d3_dag.dagHierarchy().id(() => "0").children(() => [])(false), /falsy data/);
  test.end();
});
