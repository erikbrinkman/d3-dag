...
export default function() {
    ...
    function columnIndexAssignmentSubtree(layers, center) {
      ...
      layers[0].forEach((node) => {
        ...
        node.columnIndex = startColumnIndex + (center ? Math.floor((subtreeWidth - 1) / 2) : 0);
      ...
      function getSubtreeWidth(node) {
        ...
        return node.children.reduce((prevVal, child) => prevVal + getSubtreeWidth(child), 0);
      }
      ...
      function assignColumnIndexToChildren(node, startColumnIndex) {
        ...
        node.children.forEach((child, index) => {
          ...
          child.columnIndex = childColumnIndex + (center ? Math.floor((widthPerChild[index] - 1) / 2) : 0);
