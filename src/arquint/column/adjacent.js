export default function() {
    ...
    function columnIndexAssignmentAdjacent(layers) {
      ...
      layers[maxNodesLayerIndex].forEach((node, index) => node.columnIndex = index);
      ...
      function fillLayerBackwards(layer) {
        ...
        if (layer.length === maxNodesCount) {
          ...
          const desiredColumnIndices = layer.map((node, index) => {
            ...
            const childrenColumnIndices = node.children.map((child) => child.columnIndex);
            if (center) {
        ...
        layer.forEach((node, index) => node.columnIndex = actualColumnIndices[index]);
      }
      ...
      function fillLayerForward(layer) {
        ...
        if (layer.length === maxNodesCount) {
          ...
          const desiredColumnIndices = layer.map((node, index) => {
            ...
            const parentColumnIndices = node.parents.map((parent) => parent.columnIndex);
            if (center) {
      ...
      function optimizeColumnIndices(desiredColumnIndices) {
        ...
        if (downShift > 0) {
          ...
          desiredColumnIndices = desiredColumnIndices.map(
            (columnIndex, index) => Math.max(columnIndex - downShift, index));
        }
