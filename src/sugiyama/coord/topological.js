...
export default function() {
  ...
  function coordTopological(layers, separation) {
    if (!layers.every(layer => 1 === layer.reduce((c, n) => c + !!n.data, 0))) {
      throw new Error("coordTopological() only works with a topological ordering");
    }

    // This takes advantage that the last "node" is set to 0
    const inds = {};
    let i = 0;
    layers.forEach(layer => layer.forEach(n => n.data || (inds[n.id] = i++)));
    layers.forEach(layer => layer.forEach(n => inds[n.id] === undefined && (inds[n.id] = i)));
